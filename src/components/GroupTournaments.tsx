...
import { Trophy, ArrowLeft, Users, X, Award, Share2 } from 'lucide-react';
    ...
    try {
      ...
      for (const roundInfo of rounds) {
        const toInsert = Array.from({ length: roundInfo.count }, (_, i) => ({
          tournament_id: tournamentId, team_a_id: null, team_b_id: null,
          score_a: 0, score_b: 0, status: 'scheduled', round: roundInfo.round,
          match_type: 'playoff', is_return_match: false, next_match_id: null,
          position_in_round: i, user_id: user.id
        }));
      ...
      const allPlayoffMatches = savedRounds.flatMap((sr, srIdx) =>
        sr.matches.map((m: any, idx: number) => ({
          ...
          scoreA: 0, scoreB: 0, played: false, round: m.round, positionInRound: idx, nextMatchId: m.next_match_id
        }))
      );
                    ...
                    <div className="flex gap-12 min-w-max p-4">
                      {(() => {
                        ...
                        return rounds.map(roundSize => {
                          const roundMatches = playoffMatches.filter(m => m.round === roundSize).sort((a, b) => a.positionInRound - b.positionInRound);
                          const roundLabel = roundSize === 2 ? 'Finale' : roundSize === 4 ? 'Semifinali' : roundSize === 8 ? 'Quarti' : roundSize === 16 ? 'Ottavi' : `Turno ${roundSize}`;
                          return (
                                  ...
                                  return (
                                    <div key={match.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 w-64 overflow-hidden hover:border-indigo-300 transition-all">
                                      <div className={`flex items-center justify-between px-4 py-3 ${match.played && match.scoreA > match.scoreB ? 'bg-indigo-50' : ''}`}>
                                        <span className={`text-sm font-bold truncate flex-1 ${match.played && match.scoreA > match.scoreB ? 'text-indigo-600' : 'text-slate-700'}`}>{match.teamA?.name || '---'}</span>
                                        <input type="number" min="0" className="w-10 h-8 text-center text-sm font-black bg-white rounded-lg border border-slate-200 outline-none ml-2" value={match.scoreA} onChange={e => updatePlayoffScore(matchIdx, parseInt(e.target.value) || 0, match.scoreB)} />
                                      </div>
                                      <div className="h-px bg-slate-100" />
                                      <div className={`flex items-center justify-between px-4 py-3 ${match.played && match.scoreB > match.scoreA ? 'bg-indigo-50' : ''}`}>
                                        <span className={`text-sm font-bold truncate flex-1 ${match.played && match.scoreB > match.scoreA ? 'text-indigo-600' : 'text-slate-700'}`}>{match.teamB?.name || '---'}</span>
                                        <input type="number" min="0" className="w-10 h-8 text-center text-sm font-black bg-white rounded-lg border border-slate-200 outline-none ml-2" value={match.scoreB} onChange={e => updatePlayoffScore(matchIdx, match.scoreA, parseInt(e.target.value) || 0)} />
                                      </div>
                                      <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 flex justify-center">
                                        <button onClick={() => openMatchReport(match)} className="text-[10px] font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-800 transition-colors">Referto Marcatori</button>
                                      </div>
                                    </div>
                                  );
                                })}
