...
export const ClubsPage = () => {
  ...
  const deleteClub = async (id: string) => {
    if (!confirm("Sei sicuro di voler eliminare definitivamente questo club?")) return;
    
    try {
      const { error } = await supabase.from('teams').delete().eq('id', id);
      if (error) throw error;
      fetchClubs(); // Aggiorna la lista dopo l'eliminazione
    } catch (err: any) {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px', color: 'white' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h1 style={{ fontSize: '24px' }}>🛡️ {editId ? "Modifica Club" : "Gestione Club"}</h1>
        <button 
          onClick={() => setShowPlayers(true)} 
          style={{ background: '#28a745', color: 'white', border: 'none', padding: '12px 20px', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 6px rgba(0,0,0,0.3)' }}
        >
